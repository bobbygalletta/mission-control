#!/usr/bin/env node
// Agent Chat Server - Separate from Mission Control
// Serves only Agent Chat on port 8790
...
const server = http.createServer(async (req, res) => {
  ...
  // For Agent Chat, we serve agent.html as the main page
  let filePath;
  if (pathname === '/') {
    filePath = path.join(DIST_DIR, 'agent.html');
  } else {
    filePath = path.join(DIST_DIR, pathname);
  }

  // Fallback to agent.html for SPA routing
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`[${new Date().toISOString()}] Agent Chat server running on http://0.0.0.0:${PORT}`);
});
